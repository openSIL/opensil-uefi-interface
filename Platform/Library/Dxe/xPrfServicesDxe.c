...
#include <Protocol/xPrfServicesDxeProtocol.h>
...
// RAS xPRF Services Start
  .CollectDimmMap             = SilCollectDimmMap,
  .GetMaxCpus                 = SilGetMaxCpus,
  .CollectCpuMap              = SilCollectCpuMap,
  .CollectCoreMcaInfo         = SilCollectCoreMcaInfo,
  ...
  .CoreMcaIpIdInstanceIdInit  = SilCoreMcaIpIdInstanceIdInit,
  .UpdateMcaFruText           = SilUpdateMcaFruText,
// RAS xPRF Services End
  .GetThreadsPerCore          = SilGetThreadsPerCore,
  .GetPStatePower             = SilGetPStatePower,
  .CreateSratApicEntry        = SilCreateSratApicEntry,
  .PStateGatherData           = SilPStateGatherData,
  .GetCratHsaProcInfo         = SilGetCratHsaProcInfo,
  .CratCacheEntry             = SilCratCacheEntry
};
 ...
 *
 * @param   Buffer  Input pointer to a buffer that will be populated with SIL_RAS_MCA_ERROR_INFO_V2.
 *
 */
VOID
SilCollectMcaErrorInfo (
...
{
  xPrfCollectMcaErrorInfo ((SIL_RAS_MCA_ERROR_INFO_V2 *)Buffer);
}
  ...
  )
{
  ...
  SilStatus = xPrfUpdateMcaFruText (RasCpuInfo);
  return TranslateStatus (SilStatus);
}
...
{
  SIL_STATUS  SilStatus;
  SilStatus = xPrfGetPStatePower(Pstate, PowerInmW);
  switch(SilStatus) {
  case SilAborted:
    DEBUG ((DEBUG_INFO, "opensil-uefi-interface unable to set xPrf PStatePower Value\n"));
    break;
  case SilNotFound:
    DEBUG ((DEBUG_INFO, "opensil-uefi-interface xPrf PStatePower cannot get CCX I2I API\n"));
    break;
  case SilPass:
    DEBUG ((DEBUG_INFO, "opensil-uefi-interface xPrf PStatePower Value %d\n", PowerInmW));
    break;
  default:
    DEBUG ((DEBUG_INFO, "opensil-uefi-interface xPrf PStatePower unknown error %d\n",PowerInmW));
    break;
  }
  ...
  )
{
  ...
  SilStatus = xPrfPStateGatherData ((SIL_PSTATE_SYS_INFO *)PStateData, PStateDataSize);
