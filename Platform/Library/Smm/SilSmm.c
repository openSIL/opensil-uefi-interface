...
)
{
  switch (SilStatus) {
    ...
    case SilOutOfBounds:          return EFI_BUFFER_TOO_SMALL;
  ...
  )
{
  ...
  DEBUG((DEBUG_INFO, "SIL SMM entry point.\n"));

  Hob = (EFI_HOB_GUID_TYPE *)GetFirstGuidHob (&gPeiOpenSilDataHobGuid);
  if (Hob == NULL) {
    DEBUG ((DEBUG_ERROR, "Unable to get first GUID HOB\n"));
    ...
    if (Status == EFI_SUCCESS) {
      DEBUG((DEBUG_INFO, " SMM Buffer allocated size = 0x%x @%lx\n", SilDataBlockSize, SilData));
    }
    ...
    if (Status != EFI_SUCCESS) {
      DEBUG ((DEBUG_ERROR, "Unable to assign SIL Base pointer in openSIL\n"));
      ASSERT_EFI_ERROR (Status);
    }
  }
